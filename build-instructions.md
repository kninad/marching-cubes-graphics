...
## Getting started
This repo shows an example of how to build a basic OpenGL program with Gui using MacOS or Linux(Ubuntu). You can use this example as starting code for future assignments. All dependencies of OpenGL are already included in this repo.

You can find the repository at https://github.com/CS6366utd/environment_setup_mac. If you are unfamiliar with git, here is what you need to do in terminal to get the starter code:
```
$ git clone https://github.com/CS6366utd/environment_setup_mac
```
This will create an environment_setup_mac folder with all the source files.

## Build Instructions

In order to ease the process of running on different platforms, we will be using CMake for our assignments. You will need a CMake installation of version 3.5+ to build the code for this assignment. It should also be relatively easy to build the assignment and work locally on your OSX. Building on Windows is currently not supported, please refers to the Windows tuitorial.
...
```
$ cd environment_setup_mac && mkdir build && cd build
$ cmake ..
$ make
```
